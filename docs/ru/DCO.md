...
__Ситуация:__ Вы редактируете файл напрямую на GitHub (через веб-интерфейс), в результате чего создаётся коммит. По умолчанию GitHub не добавляет “Signed-off-by” к коммитам, сделанным через веб-интерфейс, и ваш Pull Request может не пройти проверку DCO из-за такого коммита.
